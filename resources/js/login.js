// ==========================================
// Halaman Login: Rive Canvas, Bottom Sheet, Portal & Form Logic
// ------------------------------------------
// Dipindah verbatim dari @push('scripts') di
// resources/views/auth/login.blade.php agar HTML awal lebih kecil,
// ter-minify, ter-cache, dan tidak memblokir render (module = deferred).
// URL aset dibaca dari <script id="login-assets">,
// SVG ikon dibaca dari <template id="login-icon-*">.
// TIDAK ada perubahan logika/perilaku di file ini.
// ==========================================

function readLoginAssets() {
    var el = document.getElementById('login-assets');
    if (!el) return {};
    try {
...
var LOGIN_ASSETS = readLoginAssets();

function loginIcon(id) {
    ...
    (function () {
        ...
        function loadRiveLibrary(callback) {
            ...
            script.src = LOGIN_ASSETS.riveJs;
            script.async = true;
        ...
        function initWhenReady() {
            ...
            var wasmUrl = LOGIN_ASSETS.riveWasm;
    ...
    function initLoginForm(root) {
        ...
        const portalIcons = {
            'id-card': loginIcon('login-icon-id-card'),
            'shield-user': loginIcon('login-icon-shield-user'),
        };
        ...
        if (pwBtn && pwInput) {
            pwBtn.addEventListener('click', () => {
                ...
                pwBtn.innerHTML = show
                    ? loginIcon('login-icon-eye-off')
                    : loginIcon('login-icon-eye');
            });
        }
